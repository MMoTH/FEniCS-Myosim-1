...
ha_avg = np.nan*np.ones((np.shape(f0_vs_time)[0],8))
tv_avg = np.nan*np.ones((np.shape(f0_vs_time)[0],8))
ha_std = np.nan*np.ones((np.shape(f0_vs_time)[0],8))
tv_std = np.nan*np.ones((np.shape(f0_vs_time)[0],8))

# calculate averages and sd for each point each beat
for i in np.arange(8):
    for j in np.arange(np.shape(f0_vs_time)[0]):
        ha_avg[j,i] = np.average(helical_angles_vs_time[j,i*2000:(i+1)*2000])
        tv_avg[j,i] = np.average(transv_angles_vs_time[j,i*2000:(i+1)*2000])
        ha_std[j,i] = np.std(helical_angles_vs_time[j,i*2000:(i+1)*2000])
        tv_std[j,i] = np.std(transv_angles_vs_time[j,i*2000:(i+1)*2000])
...
'ha_std_b5':ha_std_b5,'ha_std_b6':ha_std_b6,'ha_std_b7':ha_std_b7,'ha_std_b8':ha_std_b8,'tv_std_b1':tv_std_b1,'tv_std_b2':tv_std_b2,'tv_std_b3':tv_std_b3, \
'tv_std_b4':tv_std_b4,'tv_std_b5':tv_std_b5,'tv_std_b6':tv_std_b6,'tv_std_b7':tv_std_b7,'tv_std_b8':tv_std_b8}
df = pd.DataFrame(d)
...
for jj in np.arange(m):
    for kk in np.arange(n):
        axs[jj][kk].plot(d['norm_dist_endo'][d['seg_number']==jj*n+kk],d['helical_angles_final'][d['seg_number']==jj*n+kk],'o',alpha=0.25)
        for i in np.arange(np.shape(bin_edges)[0]-1):
            tempy = d['helical_angles_final'][(d['seg_number']==jj*n+kk) & (d['right_bin_marker']==i+1)]
        ...
        axs3[jj][kk].plot(d['norm_dist_endo'][d['seg_number']==jj*n+kk],d['transv_angles_final'][d['seg_number']==jj*n+kk],'o',alpha=0.25)
        ...
        axs4[jj][kk].plot(['beat 1','beat 2','beat 3','beat 4','beat 5','beat 6','beat 7','beat 8'],[np.average(d['ha_std_b1'][d['seg_number']==jj*n+kk]),np.average(d['ha_std_b2'][d['seg_number']==jj*n+kk]), \
        np.average(d['ha_std_b3'][d['seg_number']==jj*n+kk]),np.average(d['ha_std_b4'][d['seg_number']==jj*n+kk]),np.average(d['ha_std_b5'][d['seg_number']==jj*n+kk]),np.average(d['ha_std_b6'][d['seg_number']==jj*n+kk]),np.average(d['ha_std_b7'][d['seg_number']==jj*n+kk]),np.average(d['ha_std_b8'][d['seg_number']==jj*n+kk])],'o')
        axs5[jj][kk].plot(['beat 1','beat 2','beat 3','beat 4','beat 5','beat 6','beat 7','beat 8'],[np.average(d['tv_std_b1'][d['seg_number']==jj*n+kk]),np.average(d['tv_std_b2'][d['seg_number']==jj*n+kk]), \
        np.average(d['tv_std_b3'][d['seg_number']==jj*n+kk]),np.average(d['tv_std_b4'][d['seg_number']==jj*n+kk]),np.average(d['tv_std_b5'][d['seg_number']==jj*n+kk]),np.average(d['tv_std_b6'][d['seg_number']==jj*n+kk]),np.average(d['tv_std_b7'][d['seg_number']==jj*n+kk]),np.average(d['tv_std_b8'][d['seg_number']==jj*n+kk])],'o')


fig6,axs6 = plt.subplots(1,2,)
for i in np.arange(np.shape(bin_edges)[0]-1):
    axs6[0].errorbar(x=(bin_edges[i+1]+bin_edges[i])/2,y=np.mean(d['helical_angles_final'][d['right_bin_marker'] == i+1]),yerr = np.std(d['helical_angles_final'][d['right_bin_marker'] == i+1]))
